...
```
主从切换的方法是：当主服务器宕机后，需要手动把一台从服务器切换为主服务器，要人工干预，费时费力，还会造成一段时间的服务不可用。所以更多时候优先考虑哨兵模式。
```

```
后台监控master是否故障，如果master故障了，根据票数自动将从库转换为主库。
```

```
redis提供了哨兵命令，哨兵是一个独立的进程。其原理是哨兵通过发送命令，等待redis服务器响应，从而监控运行多个redis实例。
如果之后，原来的master回来了，那它也只能变成新matser的slave。
```

```
哨兵模式优点：
1.哨兵模式基于主从复制，继承了主从复制的优点。
2.主从可以切换，故障可以转移，系统的可用性回更好。
3.哨兵模式可以说是主从复制的升级，手动变自动。

哨兵模式缺点：
1.不好在线扩容，集群容量一旦到达上限，在线扩容就十分麻烦。
2.实现哨兵模式其实是很麻烦的，配置由很多选择。
...
```
缓存穿透概念：
用户想要查询一个数据，发现redis中没有，也就是缓存没有命中，于是就向数据库查询，发现数据库里也没有，于是本次查询失败。当请求很多的时候，缓存都没有命中，都去查了数据库，给数据库造成了很大的压力，这就是缓存穿透。
```
...
布隆过滤器
布隆过滤器是一种数据结构，对所有可能查询的参数以hash形式存储，先在控制层校验，不符合则丢弃，从而避免了对db的压力。

缓存空对象
当db没有这个数据后，即使返回空对象也将其缓存起来，同时会设置一个过期时间，之后再访问这个数据会从缓存中获取。
但这种方法会存在两个问题：
1.如果空值能够被缓存起来，这就意味着需要更多的空间存储更多的键；
2.即使对空值设置了过期时间，缓存和db会有一段时间的数据不一致；
```

```
缓存击穿概念：
是指一个热点key,高并发地访问这个key,当这个key在失效地瞬间，持续地高并发访问就会穿过缓存直接到达db。
当某个key在过期的瞬间，高并发地访问，这类数据一般是热点数据。由于缓存过期，会同时访问数据库来查询最新的数据并写回缓存，会导致数据库压力瞬间过大。
```
...
加互斥锁
分布式锁：使用分布式锁，保证对每个key同时只有一个线程去查询后端服务，其他线程没有获得分布式锁的权限，因此只需要等待即可。这种方法将高并发的压力转移到了分布式锁，因此对分布式锁的考验很大。
```
...
1.redis高可用
多加几台redis服务器，一台挂掉后其他可以继续工作；
2.限流降级
在缓存失效后，通过加锁或队列来控制数据库读写缓存的线程数量，比如对某个key只允许一个线程读db和写入缓存，其他线程等待；
3.数据预热
在正式部署前，将可能大量访问的数据先写入缓存。并设置不同的过期时间，让缓存失效的时间尽量均匀,避免在某一时间集中失效。
```
